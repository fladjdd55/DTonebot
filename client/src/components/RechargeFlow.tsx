...
// Import validator functions (now operate on passed array)
...
import type { Country } from '../shared/countryValidator'; // Use the type from the shared file
...
export default function RechargeFlow() {
  ...
  // --- ASYNCHRONOUS DATA SOURCE (FIX) ---
  const { countries, loading: countriesLoading, error: countriesError } = useCountries();

  // Filter countries based on the LIVE array
  const filteredCountries = useMemo(() => {
    // FIX: Pass the LIVE 'countries' array to the filter function
    // And sort it using the utility function
    const sorted = getAllCountries(countries);
    return filterCountries(sorted, searchQuery);
  ...
  const handleLookupAndFetch = async () => {
    // Ensure local validation passed
  ...
  // RENDER FIX: Wait for countries to load
  if (countriesLoading) {

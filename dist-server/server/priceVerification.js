...
exports.priceVerificationService = {
    ...
    getAdjustedMin(product) {
        return pricingService_1.pricingService.getSafeMinAmount(product);
    },
    async verifyProductPrice(productId, paidAmount, // The amount the user PAID (in USD)
    paidCurrency) {
        try {
            ...
            if (product.type && product.type.toUpperCase().includes('RANGE')) {
                ...
                const { minRequired } = pricingService_1.pricingService.calculatePrice(product, product.minAmount || 0);
            ...
            const { finalCharge } = pricingService_1.pricingService.calculatePrice(product);
    ...
    async getProductPrice(productId) {
        try {
            ...
            const adjustedMin = pricingService_1.pricingService.getSafeMinAmount(product);
            return {
                success: true,
